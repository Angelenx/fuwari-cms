...
首次打开 http://localhost:4321/admin/login 为用户名 `admin` 设密码（`users` 表为空时）。登录后可在 `/admin/profile` 改头像、简介、三链和首页 banner，在 `/admin/site` 改站点标题、页脚和默认语言（中/英），在 `/admin/about` 改 About 正文（未写入 D1 的字段沿用 `src/config.ts`）。若本地还留着阶段 3 的种子管理员，先清空再走设密页：
